...
namespace mesh_processing
{
    ...
    bool ReadVTK2(const std::string &filename,
                  std::vector<std::vector<double>> &out_vertices,
                  std::vector<std::vector<size_t>> &out_cells,
                  std::vector<int> &out_cell_types)
    {
        ...
        std::string line;
        std::getline(file, line);
        TrimSpaceHeadAndTail(line);
        if (line != "# vtk DataFile Version 3.0"
            &&line != "# vtk DataFile Version 2.0"
            &&line != "# vtk DataFile Version 1.0")
        {
            std::cerr << "[Error] Unsupported VTK version. Expected '# vtk DataFile Version 3.0'.\n"
                      << std::endl;
            return false;
        ...
        cell_types.reserve(num_cell_types);

        int type_value;
        while (cell_types.size() < num_cell_types && file >> type_value) {
            cell_types.push_back(type_value);
        }

        if (cell_types.size() != num_cell_types) {
            std::cerr << "[Error] CELL_TYPES read mismatch: expected " 
                    << num_cell_types << " got " << cell_types.size() << ".\n";
            return false;
        }
